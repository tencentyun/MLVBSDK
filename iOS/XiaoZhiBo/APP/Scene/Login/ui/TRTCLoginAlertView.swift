//
//  TRTCLoginAlertView.swift
//  TXLiteAVDemo
//
//  Created by gg on 2021/4/19.
//  Copyright © 2021 Tencent. All rights reserved.
//

import UIKit

class TRTCLoginCountryAlert: TRTCLoginAlertContentView {
    
    lazy var dataSource: [TRTCLoginCountryModel] = {
        let list = TRTCLoginCountryModel.loginCountryList
        return list
    }()
    ...
    var didSelect: ((_ model: TRTCLoginCountryModel)->())?
    ...
    override func bindInteraction() {
        ...
        tableView.register(TRTCLoginCountryListCell.self, forCellReuseIdentifier: "TRTCLoginCountryListCell")
        tableView.dataSource = self
        tableView.delegate = self
    }
}

extension TRTCLoginCountryAlert: UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return dataSource.count
    }
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "TRTCLoginCountryListCell", for: indexPath)
        if let scell = cell as? TRTCLoginCountryListCell {
...
extension TRTCLoginCountryAlert: UITableViewDelegate {
...
class TRTCLoginCountryListCell: UITableViewCell {
    
    var model: TRTCLoginCountryModel? {
        didSet {
...
class TRTCLoginAlertContentView: UIView {
...
class TRTCLoginCountryModel: NSObject {
    let code: String
    ...
    static var loginCountryList: [TRTCLoginCountryModel] {
        get {
            let list = loadLoginCountryJson()
            var res : [TRTCLoginCountryModel] = []
            list.forEach { (dic) in
                if let code = dic["code"] as? Int, let en = dic["en"] as? String, let zh = dic["zh"] as? String {
                    let model = TRTCLoginCountryModel(code: code, displayEN: en, displayZH: zh)
                    res.append(model)
                }
            }
            return res
...
public extension String {
    static func isChineseLanguage() -> Bool {
...
fileprivate extension String {
    static let titleText = LoginLocalize("Demo.TRTC.Login.countrycode")
}
