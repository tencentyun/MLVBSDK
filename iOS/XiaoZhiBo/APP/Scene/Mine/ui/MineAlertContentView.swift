...
class MineUserIdEditView: MineAlertContentView {
    ...
    lazy var textField: UITextField = {
        ...
        textField.backgroundColor = UIColor( "F4F5F9")
    ...
    @objc func confirmBtnClick(btn: UIButton) {
        ...
        guard let name = textField.text, name.count > 0 else { return }
        if let userModel = ProfileManager.sharedManager().currentUserModel{
            HttpLogicRequest.userUpdate(currentUserModel: userModel, name: name ,success:  { [weak self] (user) in
                guard let self = self else { return }
                self.dismiss()
            } ,failed: { (errocde,errorMessage) in
                debugPrint("sms set profile error")
            })
        }
//        ProfileManager.sharedManager().setNickName(name: name) { [weak self] in
//            guard let `self` = self else { return }
//            self.dismiss()
...
fileprivate extension String {
    static let changeUseridText = MineLocalize("Demo.TRTC.Portal.changenickname")
    static let confirmText = MineLocalize("Demo.TRTC.Portal.confirm")
    static let useridPlaceholderText = MineLocalize("Demo.TRTC.Portal.enterusername")
    static let useridDescText = MineLocalize("Demo.TRTC.Portal.limit20count")
}
