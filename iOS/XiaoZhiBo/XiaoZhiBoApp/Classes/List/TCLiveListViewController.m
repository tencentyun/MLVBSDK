...
@interface TCLiveListViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,MLVBLiveRoomDelegate>

@property (strong) TCLiveListMgr *liveListMgr;
/// 直播列表
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [TCLiveListMgr sharedMgr].liveRoom.delegate = self;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.liveArray.count > index) {
        TCLiveInfo *live = self.liveArray[index];
...
- (NSArray *)mergeResult:(NSArray *)result {
    ...
    NSArray *existArray = [self.liveArray bk_map:^id(TCLiveInfo *obj) {
        return obj.playurl;
    }];
...
- (void)listDataUpdated:(NSNotification *)noti {
    ...
    for (TCLiveInfo* item in self.liveArray)
    {
