...
- (void)initLogicView {
    ...
    _logicView = [[TCPlayDecorateView alloc] initWithFrame:frame liveInfo:self.liveInfo withLinkMic: NO];
...
-(BOOL)startPlay {
    ...
    if(_txVodPlayer != nil)
    {
        if (self.liveInfo.type == TCLiveListItemType_UGC) {
...
- (void)clickShare:(UIButton *)button {
    [TCUtil shareLive:_liveInfo currentViewController:self];
}
