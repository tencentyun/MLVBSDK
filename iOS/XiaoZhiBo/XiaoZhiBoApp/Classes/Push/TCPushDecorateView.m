...
-(void) initUI
{
    ...
    //***
    //音乐 layout
    [_vMusicPanel sizeWith:CGSizeMake(self.width, 315)];
    ...
    [_vAudioEffectPanel sizeWith:CGSizeMake(self.width, 255)];
    ...
    [_labPitchForBGM alignParentTopWithMargin:30];
    [_labPitchForBGM alignParentLeftWithMargin:15];
    
    [_sldPitchForBGM sizeWith:CGSizeMake(270, 20)];
    [_sldPitchForBGM alignParentTopWithMargin:30];
    ...
    for (int i=0; i<_audioEffectViewArry2.count; ++i) {
        ...
        [btn sizeWith:CGSizeMake(40,40)];
        [btn layoutBelow:_labVolumeForVoice margin:60 + 40*rowNum];
        float margin = 15 + (btn.width + (self.width-(6*btn.width + 30))/(6-1))*(i % 6);
        [btn alignParentLeftWithMargin:margin];
    }
    
    [_labAudioEffect sizeWith:CGSizeMake(self.width, 50)];
    [_labAudioEffect layoutBelow:[_audioEffectViewArry2 objectAtIndex:0] margin:25];
