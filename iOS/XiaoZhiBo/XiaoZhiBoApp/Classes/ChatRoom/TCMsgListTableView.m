...
- (instancetype)initWithFrame:(CGRect)frame style:(UITableViewStyle)style liveInfo:(TCRoomInfo *)liveInfo {
    if (self = [super initWithFrame:frame style:style]) {
        self.dataSource = self;
        self.delegate   = self;
        self.separatorStyle = UITableViewCellSeparatorStyleNone;
...
- (void)getGroupMembers {
    ...
    if (TCRoomListItemType_Record == _liveInfo.type)
...
- (BOOL)isAlreadyInAudienceList:(TCMsgModel *)model {
    ...
    for (TCUserProfileData *data in _dataArray) {
...
- (void)refreshAudienceList:(TCMsgModel *)model {
    ...
    if (model.msgType == TCMsgModelType_MemberEnterRoom) {
        TCUserProfileData *infoData = [[TCUserProfileData alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_dataArray.count > indexPath.row) {
        TCUserProfileData *msgModel = (TCUserProfileData *)_dataArray[indexPath.row];
        [cell refreshWithModel:msgModel];
    }
    return cell;
}
