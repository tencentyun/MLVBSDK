...
extension MineRootView : UITableViewDelegate {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case .privacy:
            guard let plistPath = Bundle.main.path(forResource: "Privacy", ofType: "plist") else {
                return
            }
            let config = LiteAVPrivacyConfig()
            config.style = .light
            config.userName = ProfileManager.sharedManager().currentUserModel?.name ?? ""
            config.userID = ProfileManager.sharedManager().currentUserModel?.userId ?? ""
            config.userAvatar = ProfileManager.sharedManager().currentUserModel?.avatar ?? "";
            config.phone = ProfileManager.sharedManager().currentUserModel?.phone ?? ""
            config.plistPath = plistPath
            let vc = LiteAVPrivacyViewController(privacyConfig: config)
            vc.hidesBottomBarWhenPushed = true
            rootVC?.navigationController?.pushViewController(vc, animated: true)
        case .agreement:
