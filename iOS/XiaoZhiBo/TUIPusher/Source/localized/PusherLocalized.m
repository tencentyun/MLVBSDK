...
NSString *TCLLocalizeFromTable(NSString *key, NSString *table) {
    return [PusherBundle() localizedStringForKey:key value:@"" table:table];
}
...
NSString *const Pusher_Localize_TableName = @"PusherLocalized";
NSString *PusherLocalize(NSString *key) {
    return TCLLocalizeFromTable(key, Pusher_Localize_TableName);
}
