...
- (void)loginWithCompletion:(void(^)(int errCode, NSString *errMsg))completion {
    [self asyncRun:^{
        [[V2TIMManager sharedInstance] login:self->_config.userID userSig:self->_config.userSig succ:^{
            if (completion) {
                completion(0, nil);
            }
...
- (void)sendNotifyMessage {
    ...
    if (_groupConversation) {
        [[V2TIMManager sharedInstance] sendGroupCustomMessage:[self dictionary2JsonData:data] to:self->_groupConversation.getReceiver priority:V2TIM_PRIORITY_NORMAL succ:^{
...
- (void)sendRoomCustomMsg:(NSString *)cmd msg:(NSString *)msg completion:(void (^)(int errCode, NSString *errMsg))completion {
    [self asyncRun:^{
        ...
        [[V2TIMManager sharedInstance] sendGroupCustomMessage:[self dictionary2JsonData:customMsg] to:self->_groupConversation.getReceiver priority:V2TIM_PRIORITY_NORMAL succ:^{
            Log(@"sendCustomMessage success");
...
- (void)createGroupWithID:(NSString *)groupID name:(NSString *)groupName completion:(void(^)(int errCode, NSString *errMsg))completion {
    ...
    [[V2TIMManager sharedInstance] createGroup:@"AVChatRoom" groupID:groupID groupName:groupName succ:^(NSString *groupID) {
...
{
    [[V2TIMManager sharedInstance] getGroupMemberList:groupID filter:V2TIM_GROUP_MEMBER_FILTER_COMMON nextSeq:0 succ:^(uint64_t nextSeq, NSArray<V2TIMGroupMemberFullInfo *> *memberList) {
