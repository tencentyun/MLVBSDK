...
Component({
  ...
  data: {
    ...
    hasPlayerEnter: false, // 是否已有远端信号进来
    CONSTANT, // 常量
    pusherContext: '',
  ...
  methods: {
    ...
    onWebRTCUserListPush: function (msg) {
      ...
      console.log("onWebRTCUserListPush.jsonDict:", jsonDic); // , self.data.userID, self.data.pushURL
      ...
      console.log("onWebRTCUserListPush pushers:", pushers, self.data.userID);
      // 这里的postErrorEvent 会提示所有已在房间里的成员，造成所有观众都收到提示，并退出房间，应该只提示最后进房间的人，问题是目前无法区分最后进房间的人。先注释。
      ...
      var tmpHasPlayerEnter = false;
      if (newUserList && newUserList.length > 0) {
        tmpHasPlayerEnter = true;
      }
      if (tmpHasPlayerEnter != self.hasPlayerEnter) {
        self.setData({
          hasPlayerEnter: tmpHasPlayerEnter
        });
      }
